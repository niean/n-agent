...
[20260712]
- FR
    - 对话：Dashboard Chat，从直接调用HttpAPI、改走Gateway，从而支持slash cmd执行(如/compact 压缩上下文) —— 副作用过大，暂不执行
    - 记忆：消息压缩Bugfix，修复summary位置错误，tail保护从20调整为10、默认关闭tail_budget
    - 对话：Chat，输入`查一下定时任务列表`，输出`orphan tool message without preceding assistant tool_use`
    - 观测：新增缓存命中率指标，覆盖到整体总览卡片(放到缓存Token之后)、会话详情卡片(放到缓存Token之后)、会话详情之API调用历史(放到缓存列之后)
    - 观测：会话详情，API调用历史-详情，工具定义 (Capability Context)默认收起、点击展开，收起/展开图标不要太显眼
    - 观测：缓存，拆分为缓存读、缓存写两个指标分别展示，涉及整体总览卡片、会话详情卡片、会话详情列表
