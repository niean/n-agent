<!-- SUMMARY: N-Agent 的产品定位、目标用户、体验原则和 MVP 范围判断准则 -->
...
- 架构可演进：MVP 只实现当前闭环，但 Domain 端口、Application 编排和 Infrastructure 实现必须便于后续扩展。
...
4. 新能力是否符合 MVP 范围，是否只保留扩展点而非提前实现完整平台能力。

优先级低的未来能力即使技术可行，也应推迟到对应迭代，避免破坏 MVP 的简洁性和可验证性。

---

## 不做什么

- 不在 MVP 中提前实现完整多 Agent 编排、后台任务平台、Cron、远程运行环境或消息网关。
...
当产品定位、目标用户、MVP 范围、Dashboard 定位或 Agent 能力优先级发生变化时更新本文件；实现事实变化先回填 knowledge，再由人工确认是否调整 PRD。