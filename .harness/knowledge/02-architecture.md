...
- Memory/Context：通过 MemoryStore 与 Summarizer 端口访问，会话、消息、工具调用、任务状态、摘要和会话级外部记忆配置的持久化细节属于 Infrastructure。Chat Session 的外部记忆 profile 必须首轮后锁定，避免同一会话内切换项目记忆导致 system prompt 前缀变化、LLM prefix cache 失效和历史语义混用。
