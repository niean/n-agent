...
- 外部记忆 Provider 管理用例：`app/application/external_memory_provider_service.py`，定义 `ExternalMemoryProviderService`（CRUD/activate/probe + `ActiveExternalMemoryReader` 端口实现）、`ActivateResult`(frozen dataclass, 含 `tool_surface_refresh_failed` 标志)、`ActiveExternalMemoryReader` Protocol（`get_active_provider_names()` 读 manager 内存，无 IO）；activate/probe 通过 `_build_adapter_config(cfg)` 合入 `cfg.base_url` + `cfg.extra_config` 构造 adapter config（base_url 是 provider 顶层字段，非 extra_config，必须显式合入否则 Dashboard 配置被静默忽略，mem0/honcho 共享此装配路径）；activate 调用 adapter.initialize 后委托 manager.swap_external_query_provider 装载；probe 对 MEM0 类型发轻量 `mem0_profile` handle_tool_call 联网验证（POST /memories/?page=1&page_size=50，2xx→OK，401/网络异常→FAILED），对 HONCHO 调用 adapter.probe()（GET /v3/workspaces/{wid}/sessions/probe/context，不 ensure_session，401/网络异常→FAILED），对 HOLOGRAPHIC 跑一次 prefetch 验证 SQLite；get_active_provider_names 委托 manager.get_active_external_query_provider_name，供 ChatCompletionService 默认 profile 派生使用
...
- ChatService 默认 profile 派生测试：`tests/application/test_chat_service_default_profile.py`，覆盖未传字段派生 active 检索记忆 provider、显式 builtin 不纳入、显式 mem0 保留、无 active 时默认 builtin
