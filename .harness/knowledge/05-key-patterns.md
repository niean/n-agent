...
## 模式四十：退出码契约与"提交后失败"的收敛点

跨进程编排里，退出码是外部唯一可观测的结果，因此每个码必须只表达一件事，且不能在错误的时刻被说出来。Config Bundle 的契约是 0 全成功 / 1 部分记录失败其余已应用 / 2 包或参数格式错误且目标未被改动 / 3 运行依赖缺失或启动失败 / 4 只读挂载源被目录占据。

这个契约会在两个地方被悄悄破坏，两者都必须显式设防：

一是 Application 层的兜底 except。CLI 把任何未捕获异常报成退 2（"目标未被改动"），但导入是按段顺序提交的 —— 段内逐行 guard 只保护"写一行"，而 guard 之外的工作（读目标现状解冲突、写单例段）一旦抛出，前面若干段已经落库，退 2 就在对运维说反话，并诱使其对一个自认为干净的目标重跑。做法：把校验与执行分开对待 —— `_validate_bundle` 在任何写入之前，继续抛异常（此时退 2 是准确的）；其后每段用 `_apply_section` 包裹，段级失败收敛为一条 `section="*"` 的 FAILED item，从而落到退 1。item 只记异常类型不记 message，避免把正在写入的取值带进报告。

二是 POSIX sh 的两个陷阱。`if ! cmd; then exit $?; fi` 里 `$?` 是取反后的 0，会把失败静默转成成功，必须写 `cmd || exit $?` 才能透传真实码。`set -e` 对 AND-OR 列表只作用于最后一条命令，因此 `x=$(helper)` 形式的赋值仍受 `-e` 约束 —— 在数据库已提交之后调用 helper（判断是否需要重启、渲染报告）必须显式兜住失败并给出安全默认值（宁可多重启一次，也不要让新记录留在未生效状态），否则 helper 的非零码会直接成为脚本退出码。

## 模式四十一：秘密四态与"缺席不等于清除"

迁移类功能中，配置字段的"没有值"至少有四种互不相同的含义，压缩成三态就必然误伤目标机的现有凭据。约定：字段缺席 = 保持目标现值不变；`""` = 明确清除；非空 = 覆盖；`null` = 导出时被脱敏，导入侧保留目标现值并记 degraded。

配套两条不变量：脱敏包不得声称包内无凭据（workspace 文件、URL、命令参数里的内嵌凭据无法结构化剥离，必须在报告里显式披露残留风险）；`--no-secrets` 的整文件跳过与按键名 marker 清值是两种手段，前者用于纯秘密文件（oss.env、token），后者用于混合文件（docker.env），不要指望 marker 覆盖前者的键名。

同时 `action`(created|updated|none) 必须与 `outcome`(OK|SKIPPED|DEGRADED|FAILED) 正交：degraded 常常意味着"报告了降级但没写"，只有 `action` 才能回答"是否真的改了目标"，因此只有 `action` 可以驱动是否重启。幂等判定同理 —— 第二次 merge 必须是 10/10 skipped 且 `action=none`。

## 模式四十二：宿主/容器职责切分与只读构造保证

凡是"在宿主上操作一个跑在容器里的服务"的能力，宿主侧只做打包、预检、文件落地和编排，绝不直接触碰 SQLite；一切 DB 读写都经容器内 CLI 进入 Application 服务，宿主 helper 脚本不 import 任何 `app.*`。

只读不能靠约定，要靠构造：导出与 `--dry-run` 统一经 `open_sqlite(path, read_only=True)`（URI `mode=ro`）打开，从而不建 schema、不建目录、不落临时文件；目标库不存在时报 unknown 而不是报错。宿主落地阶段，单文件只读挂载源（policy、token）必须在任何 compose 调用之前以文件形式存在，否则 Docker 会替你建成目录；该路径已被目录占据时退 4 且不删除既有目录。包内自带的安装脚本一律不执行并在报告中声明已忽略，避免包获得执行权。导入期间用一个 B 类 env-only 开关（`migration_maintenance`）关停后台写入者（scheduler、飞书长连接），而不是改写并恢复用户自己的开关取值。

## 模式四十三：会话 E2E 数据集的通道回放与 manifest 权威清理

会话级 E2E（tests/e2e/conversations_runner.py）把真实会话固化为 JSON 数据集，经统一 ChannelDriver 抽象按来源通道回放（dashboard HTTP SSE / CLI gateway / ACP stdio 子进程），回放产物全部以 `e2e-<run_id>-` 前缀做逻辑隔离，LLM Judge 判定消息质量、SideEffectVerifier 核验任务终态/制品注册/workspace 文件 sha256。

隔离与清理的关键构造：(1) ManifestRecorder 是删除的唯一权威——所有可能产生持久态的路径（会话、任务、制品、sandbox、workspace 文件）在创建时即登记 intent/upsert，Cleaner 只删 manifest 登记项，不按前缀扫描现网；(2) workspace 文件删除前重算 sha256 与登记值比对，不一致拒绝删除（防误删用户同名文件），预检保证开跑前路径缺席从而运行产物可归因；(3) 无删除能力的写入（遥测计数、mem0、外部 gateway 键）显式列入报告 exemptions 而非静默漏删；(4) `--cleanup-only <manifest>` 支持只清理模式且幂等（已删除项视为成功）；(5) Judge 输入与报告统一经 `_scrub_secrets` 脱敏，数据集密码只用占位符，真实密码仅经 `docker exec -e 变量名` 注入不进 argv。

中断恢复（进程被 kill 后残留）：cli 渠道的 Gateway 入口键 `conversation_id`（`e2e-<run_id>-<case_id>`）随 session intent 一并登记——prepare 的第一条子进程命令已在服务端建成 `cli-<uuid>` 会话、第二条 `--browse` 才解析真实 ID，进程在该窗口内被 kill 时 manifest 只剩无 id 的 intent；Cleaner `_resolve_intent_sessions` 对无 id 且带 conversation_id 的 intent session 用只读 `--browse` 重新解析：恰一个 ID 回填后走正常删除，零个视为从未建成直接删除态，多个拒绝猜测记 failed（acp/dashboard 渠道 intent 分别因 upsert 窗口极小、id 预生成而不需要此路径）。会话删除的任务侧级联见模式十六：`delete_task` 清理 worker 会话按显式 `execution_session_id` -> 无显式且无 origin 时清理派生 `task-{uuid5(task.id)}`（kanban/CLI/API 任务的 worker 会话，不级联会永久泄漏）-> origin_session_id 存在时跳过（worker 复用用户 Chat 会话，禁删）。

## 模式四十四：E2E 宿主编排脚本的 RUNNING 任务清理契约

创建真实 Task（异步 LLM worker）的 E2E 脚本（tests/e2e/artifacts.sh）cleanup 必须同时满足三层约束，缺一层就会泄漏任务 + 派生 worker 会话（task-{uuid5}）：

1. cancel/delete 必须走服务端 HTTP 路由（`POST /chat/tasks/{id}/cancel`、`DELETE /chat/tasks/{id}`），禁止 `docker exec n-agent task cancel/delete`——CLI 是独立进程，其 run_service 无 in-process worker handle，cancel 只写 terminate_requested 事件等租约回收，而 cancel_task 对任何非异常结果都返回 `{"status":"cancelled"}`，造成"已取消"假象。
2. 状态轮询必须从 `.task.status` 提取（GET /chat/tasks/{id} 响应包装在 `.task` 下；裸 `.status` 恒 null，等待逻辑形同虚设），并按终态集合（succeeded/failed/cancelled/expired）判定，而非"非 running"。
3. 任何 `docker restart` 步骤之前必须先把自建任务收敛到终态（HTTP cancel + 等待终态）——重启杀死 in-process worker 后，孤儿 RUNNING 任务在无后台回收循环（D069）下只能等 900s 租约过期才被 `_recover_stale_executions` 回收，cleanup 窗口内无解。

删除失败必须 WARN 可见，禁止 `|| true` 静默吞错；Section 写入的 workspace 文件须登记（track_workspace_file）并在 cleanup 删除。该契约由 tests/e2e/test_artifacts_sh.py 静态合同测试锁定（直接读脚本源码正则断言，防回归）。
