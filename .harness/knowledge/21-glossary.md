...
- PublishedArtifact：Artifact 的不可变发布快照实体（`app/domain/artifact.py`，frozen dataclass）。artifact_id nullable，支持源 Artifact 删除后快照独立存活（FK ON DELETE SET NULL）。新增 published_revision_id（nullable，记录被发布的 Revision；新发布非空，旧数据 null 按 outdated 处理）。快照字段（snapshot_name/kind/mime/content_ref/inline_content/size/checksum）在发布时固化，仅 status 与 revoked_at 可变。is_active 属性判定是否未撤销。
...
- ArtifactRevision：Artifact 的不可变内容版本快照（`app/domain/artifact.py`，frozen dataclass）。含 revision_number（per-artifact 从 1 递增）、parent_revision_id、rollback_from_revision_id、kind/mime、inline_content 或 content_ref（XOR）、checksum/size、change_summary、created_by/at。内容更新只追加新 Revision 并移动 Artifact.current_revision_id，不覆盖历史。无 Revision 的 legacy Artifact 首次写返回 artifact_migration_incomplete/503（只读仍可用）。
- current_revision_id：Artifact 指向当前 Revision 的指针（artifacts 表列）。内容更新/rollback 在单 BEGIN IMMEDIATE 事务内 CAS re-verify 后移动。详情与列表返回该字段作为内容更新 CAS 令牌来源。
- expected_revision_id / CAS（compare-and-swap）：内容更新令牌。内容 PATCH 携带 JSON body expected_revision_id 或 If-Match header（quoted strong ETag）恰一，service 校验 == current_revision_id，不一致返回 artifact_revision_conflict/409 不静默覆写。两者皆空 -> 409；分歧 -> 422。metadata-only PATCH 不带令牌。rollback 同款 CAS（expected_revision_id）。
- publish_sync_state：Artifact 派生同步状态（不单独持久化）：unpublished（无 active publish）、current（active publish 的 published_revision_id == 当前 Revision）、outdated（active publish 指向旧 Revision）。创建新 Revision 不撤销 active publish（变 outdated、旧公链仍 200），重新发布才原子切换到当前 Revision。outdated 仍是 Artifact.status=published，差异只经 publish_sync_state 表达。
- rollback：以操作前 current 为 parent、以目标为 rollback_from 创建新当前 Revision（编号连续、完整历史可查、共享内容不被提前删除、可再次回滚）。POST `/chat/artifacts/{id}/rollback` body {target_revision_id, expected_revision_id, change_summary}。
- diff：比较同一 Artifact 任意两个文本 Revision（unified diff，头与 context_lines 确定）；二进制对返回 checksum/mime/size 摘要；文本与二进制混合 422；超限 artifact_diff_too_large/413（不返回半截）。POST `/chat/artifacts/{id}/diff` body {from_revision_id, to_revision_id, context_lines}。
- ArtifactExporterPort / OfficeArtifactExporter：Domain 导出端口（`app/domain/artifact_exporter.py`）与 Infrastructure Office 实现（`app/infrastructure/artifact/exporters.py`）。DOCX/PPTX/XLSX 由 python-docx/python-pptx/openpyxl 生成（格式库仅在 exporters.py 导入，Domain/Application/Tools 不得导入）。capabilities 逐格式返回，未支持返回 artifact_export_unsupported；历史 revision_id 与 export 使用同一版本。ArtifactExporterConfig(max_output_bytes) 注入 ArtifactService.exporter。
- artifact_* 工具：普通 Chat 暴露的 8 个 Agent 原生制品工具（`app/application/artifact_tools.py` 定义、`app/infrastructure/tools/artifact_management.py` ArtifactToolExecutor 实现）：create/list/read/update/list_revisions/diff/rollback/publish。风险等级 create/update/rollback=CONFIRM、publish=DANGEROUS、其余 SAFE。可信溯源（session_id/run_id/actor_id）仅取自 ToolExecutionContext，工具参数不接受也不覆盖。artifact_list 按 ctx.session_id 过滤（source_session_id 隔离）。
- ui.artifact 卡片：写工具（artifact_create/update/rollback/publish）成功后由 Chat 编排层（agent_graph.execute_tools）持久化的系统消息（role=system、name=ui.artifact），metadata 统一 {artifact_id, revision_id, name, kind, revision_number, publish_sync_state}。失败/审批拒绝/只读工具不写卡片。best-effort，append 失败不影响工具结果。
- artifact_guidance：artifacts_enabled 时注入 system prompt 的制品工具使用指引（`app/application/prompt_builder.ARTIFACT_GUIDANCE`），经 main.py -> AgentGraphRunner(artifact_guidance=) -> ContextService -> build_system_prompt(artifact_guidance=) 线程装配（与 browser_guidance 同款），引导 LLM 优先用 Artifact 工具产出/修改/比较/恢复/发布、artifact_list 选候选不猜测、artifact_read 处理脱敏。
...
- Activated Skills（会话级激活技能）：用户对当前请求希望被哪些 Skill 显式启用的声明。Dashboard 对话输入框工具条"记忆"之后的"技能"popover 选择其内容，选择按会话持久化到 `localStorage['nagent.chat.activated-skills']`、随对话请求以 `options.activated_skills` 下传，最终在 system prompt 中生成 `## Activated Skills` 段落指示模型先 `skill_view(name)` 再执行。与"Available Skills 发现索引"职责分离：发现索引是 LLM 自助查找的入口，Activated Skills 是用户显式声明的意图。详见模式三十七、spec-260824-chat-composer-skill.md。
