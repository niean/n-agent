...
父 Agent 可通过 `delegate_agents` 把工作拆给多个子 Agent，并等待结果返回。

```text
父 Agent 调用 delegate_agents
  -> 校验服务端授权、子任务、工具、预算和超时
  -> 按 delegation_key 创建或接回同一批委派
  -> 子 Agent 在隔离会话中并行执行
  -> 按 join_policy 汇合结果
  -> 脱敏后作为 ToolResult 返回父 Agent
```

多Agent委派入口，包括实时对话、Task worker。多Agent委派禁止递归，只支持一层；子 Agent 看不到父会话，也不能继续委派、审批任务或使用管理写工具。
