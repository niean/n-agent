...
- `artifacts`：id PK、name、kind（CHECK 11 kind 枚举）、mime、content_ref（nullable）、inline_content（nullable）、size、checksum（`sha256:` + 64 hex）、source_kind、source_ref、source_context_ref（nullable）、source_session_id（nullable，任务制品的可查询会话关联，由注册时 task_session_resolver 解析 task_execution_session_id 写入）、summary（DEFAULT ''）、classification（nullable）、labels_json（nullable）、status（DEFAULT 'draft'）、created_by（DEFAULT ''）、created_at、updated_at；唯一索引 `idx_artifacts_source ON (source_kind, source_ref)` 防重复注册；查询索引 `idx_artifacts_updated_at_id ON (updated_at, id)`（游标分页）、`idx_artifacts_source_kind ON (source_kind)`、`idx_artifacts_kind ON (kind)`、`idx_artifacts_status ON (status)`、`idx_artifacts_source_session_id ON (source_session_id)`（对话面板按会话查制品）；source_session_id 列由幂等迁移 `_migrate_add_source_session_id`（PRAGMA table_info 检查 + ALTER TABLE ADD COLUMN）补入既有库
