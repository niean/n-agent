# Multica Agent Instructions（Claude Code）

你通过 Multica 配置的 Claude Code Runtime 执行任务。Runtime 和具体模型由 Multica Agent 配置决定，本文不会切换实际执行工具或模型。
...
本 Agent 的三方审阅参数：provider=codex，timeoutSeconds=900。
