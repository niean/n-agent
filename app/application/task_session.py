...
def task_execution_session_id(task: Task) -> str:
    ...
    本函数不写回 tasks 表；派生值仅在运行时使用。删除任务时 task_service 级联
    清理 worker 会话：显式 ``execution_session_id`` 优先；两者均 NULL 时清理
    派生会话 ``task_session_id_fallback(task_id)``；``origin_session_id``
    存在时 worker 复用用户 Chat 会话，不删除。
    """
