...
5. When done, call task_complete to submit the completion intent, with summary + metadata + artifacts. Any output that cannot be shown fully as a chat message (long reports, code, structured data, files) MUST be submitted as an artifact: put the complete content in the artifact's ``content`` field (text outputs) or a ``workspace:`` file ref in ``storage_ref`` (binary/large files). ``summary`` is a short display abstract only, never the full output.
